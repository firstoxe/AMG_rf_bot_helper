...
from filters.all_item_check.guild_sklad import GuikdSkladCheck
from filters.all_item_check.back_pack_item import BackPackItemCheck
from filters.all_item_check.equip_item_check import EquipItemCheck
...
@rate_limit(0, 'auc_item_check')
@dp.message_handler(EquipItemCheck())
async def bot_Equip_item_ch(message: types.Message):
    pool: Connection = db
    list_item =[]
...
@rate_limit(0, 'auc_item_check')
@dp.message_handler(BackPackItemCheck())
async def bot_BackPack_item_ch(message: types.Message):
    pool: Connection = db
    list_item =[]
    list_item.append(f'{await helper_user(message.from_user.id)} держи:\n')
    for item in demojize(message.text).splitlines():
...
@rate_limit(0, 'auc_item_check')
@dp.message_handler(GuikdSkladCheck())
async def bot_guild_item_ch(message: types.Message):
    try:
        pool: Connection = db
        list_item =[]
        list_item.append(f'{await helper_user(message.from_user.id)} держи:\n')
...
async def bot_command_item(message: types.Message):
    pool: Connection = db
    if message.text.count(' ') > 0:
        ...
        record: Record = await pool.fetchrow('SELECT * FROM test.public.recipes '
                                          'where name=$1 or name_a = $1 or name_b = $1', name_rec.lower())
        await message.answer(f'{await helper_user(message.from_user.id)} держи: '
...
async def stat_for_item(item_stat,record,name_rec):
    ...
    if record:
        ...
        if record[6] is not None:
            result = f'{result}:crystal_ball: {i_exp:.{2}f}%[<b>{i_exp-record[6]:.{2}f}</b>]  '
        return emojize(f'<b>{name_rec}</b> +{talik}{talik2}\n{result}')
